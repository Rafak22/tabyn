...
if not GROQ_API_KEY:
    print("لازم تحطين GROQ_API_KEY قبل التشغيل!")
...
def update_suspicious_account(handle: str, claim: str, was_false: bool):
    ...
    if was_false:
        ...
        record["claims"].append({"claim": claim[:100], "date": datetime.now().isoformat()})
...
def node_check_account(state: AgentState) -> AgentState:
    ...
    reputation = check_suspicious_account(handle) if handle else {"is_suspicious": False, "false_count": 0, "total": 0}
    log = f"[CHECK] @{handle or 'غير محدد'}: {'مشبوه' if reputation['is_suspicious'] else 'نظيف'}"
    print(f"   {log}")
...
def node_retrieve(state: AgentState) -> AgentState:
    ...
    for query in state["queries"]:
        for article in search_kb(query):
...
def node_analyze(state: AgentState) -> AgentState:
    ...
    if not state["articles"]:
        analysis = {
            "thought": "لا توجد أدلة", "observation": "لم أعثر على مقالات ذات صلة",
            "defense_argument": "لا يوجد دليل", "prosecution_argument": "غياب الأدلة لا يعني الخطأ",
            "verdict": "nei", "confidence": 0.0,
        ...
        for i, a in enumerate(state["articles"], 1):
            evidence_text += f"\nالدليل {i}:\n- المصدر: {a['source']}\n- التاريخ: {a.get('date','غير محدد')}\n- العنوان: {a['title']}\n- النص: {a['body'][:300]}\n"
        rep = state.get("account_reputation", {})
        if rep.get("is_suspicious"):
            evidence_text += f"\nتحذير: هذا الحساب نشر {rep['false_count']} ادعاءات كاذبة سابقاً!\n"
        response = call_ai(FACT_CHECKER_SYSTEM_PROMPT, f"الادعاء: {state['claim']}\n\nالأدلة المتاحة:\n{evidence_text}", temperature=0.2)
...
def node_should_retry(state: AgentState) -> str:
    if state["analysis"]["confidence"] < CONFIDENCE_THRESHOLD and state.get("loop_count", 0) < MAX_RETRIEVAL_LOOPS:
        return "retry"
    return "done"
...
def node_finalize(state: AgentState) -> AgentState:
    ...
    if state.get("source_handle"):
        update_suspicious_account(state["source_handle"], state["claim"], analysis["verdict"] == "confirmed_false")
    verdict = {
...
def build_agent_graph():
    ...
    graph.add_conditional_edges("analyze", node_should_retry, {"retry": "increment_loop", "done": "finalize"})
...
app = FastAPI(title="Tabayyan", description="نظام تحقق ذكي من الأخبار العربية", version="1.0.0")
...
@app.get("/")
def root():
    return {"name": "Tabayyan", "groq_configured": bool(GROQ_API_KEY), "docs": "/docs"}

@app.get("/healthz")
def health():
    return {"status": "ok", "groq_configured": bool(GROQ_API_KEY), "kb_size": "11,539"}

@app.get("/stats")
def get_stats():
    """إجمالي الادعاءات المفحوصة منذ بدء التشغيل."""
    total = sum(r.get("total_submissions", 0) for r in SUSPICIOUS_ACCOUNTS.values())
    return {"total_claims": total}

@app.post("/verify", response_model=VerdictResponse)
def verify_claim(request: ClaimRequest):
    return _run_verification(claim=request.claim, source_handle=request.source_handle)

@app.post("/verify-image", response_model=VerdictResponse)
async def verify_image_claim(image: UploadFile = File(...), source_handle: Optional[str] = Form(None)):
    ...
    return _run_verification(claim=extracted_claim, source_handle=source_handle, from_image=True, extracted_text=extracted_claim)

def _run_verification(claim, source_handle=None, from_image=False, extracted_text=None):
    start = time.time()
    print(f"\n{'='*60}\n[NEW CLAIM] {claim[:60]}...\n[SOURCE]    {source_handle or 'غير محدد'}\n{'='*60}")
    try:
        final_state = agent_graph.invoke({"claim": claim, "source_handle": source_handle, "loop_count": 0, "trace_log": []})
        verdict     = final_state["final_verdict"]
        elapsed     = round(time.time() - start, 2)
        print(f"\n[DONE] انتهى في {elapsed} ثانية\n")
        return VerdictResponse(
            claim=claim, verdict=verdict["verdict"], confidence=verdict["confidence"],
            justification=verdict["justification_ar"], citations=verdict.get("citations", []),
            defense_argument=verdict.get("defense_argument", ""), prosecution_argument=verdict.get("prosecution_argument", ""),
            thought=verdict.get("thought", ""), account_reputation=final_state["account_reputation"],
            loop_count=final_state.get("loop_count", 0), trace_log=final_state["trace_log"],
            elapsed_seconds=elapsed, extracted_from_image=from_image, image_extracted_text=extracted_text,
        )
...
def list_suspicious_accounts():
    suspicious = [
        {"handle": h, "false_count": r["false_count"], "total": r.get("total_submissions", 0), "error_rate": r.get("error_rate", 0)}
        for h, r in SUSPICIOUS_ACCOUNTS.items() if r["false_count"] >= 2
    ]
...
def admin_update_tweets(request: AdminRequest):
    ...
    try:
        ...
        with open(tweets_path, encoding="utf-8") as f:
            tweets = json.load(f)
        return {"success": True, "total_tweets": len(tweets), "log": ["تم التحديث بنجاح!"]}
    except Exception as e:
